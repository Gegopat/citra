...
bool InitFFmpegDL() {
    ...
    auto dll_util{};
    std::unique_ptr<HMODULE, decltype(&FreeLibrary)> dll_util{LoadLibrary("avutil-56.dll"),
                                                              FreeLibrary};
    if (!*dll_util) {
    ...
    std::unique_ptr<HMODULE, decltype(&FreeLibrary)> dll_codec{LoadLibrary("avcodec-58.dll"),
                                                               FreeLibrary};
    if (!dll_codec) {
        auto error_message_id{GetLastError()};
        LPSTR message_buffer{};
        auto size{FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                                     FORMAT_MESSAGE_IGNORE_INSERTS,
                                 nullptr, error_message_id,
                                 MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&message_buffer,
                                 0, nullptr)};
        std::string message(message_buffer, size);
        LocalFree(message_buffer);
