...
bool InitFFmpegDL() {
    ...
    if (!avcodec_send_packet_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function avcodec_send_packet");
        return false;
    }
    avcodec_receive_frame_dl =
        FuncDL<int(AVCodecContext*, AVFrame*)>(dll_codec.get(), "avcodec_receive_frame");
    if (!avcodec_receive_frame_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function avcodec_receive_frame");
        return false;
    }
    avcodec_register_all_dl = FuncDL<void()>(dll_codec.get(), "avcodec_register_all");
    if (!avcodec_receive_frame_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function avcodec_register_all");
        return false;
    }
    av_get_bytes_per_sample_dl =
        FuncDL<int(AVSampleFormat)>(dll_util.get(), "av_get_bytes_per_sample");
    if (!av_get_bytes_per_sample_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_get_bytes_per_sample");
        return false;
    }
    av_frame_alloc_dl = FuncDL<AVFrame*()>(dll_util.get(), "av_frame_alloc");
    if (!av_frame_alloc_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_frame_alloc");
        return false;
    }
    av_frame_free_dl = FuncDL<void(AVFrame**)>(dll_util.get(), "av_frame_free");
    if (!av_frame_free_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_frame_free");
        return false;
    }
    av_packet_alloc_dl = FuncDL<AVPacket*()>(dll_codec.get(), "av_packet_alloc");
    if (!av_packet_alloc_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_packet_alloc");
        return false;
    }
    av_packet_free_dl = FuncDL<void(AVPacket**)>(dll_codec.get(), "av_packet_free");
    if (!av_packet_free_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_packet_free");
        return false;
    }
    av_parser_init_dl = FuncDL<AVCodecParserContext*(int)>(dll_codec.get(), "av_parser_init");
    if (!av_parser_init_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_parser_init");
        return false;
    }
    av_parser_parse2_dl =
    ...
    if (!av_parser_parse2_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_parser_parse2");
        return false;
    }
    av_parser_close_dl = FuncDL<void(AVCodecParserContext*)>(dll_codec.get(), "av_parser_close");
    if (!av_parser_close_dl) {
        LOG_ERROR(Audio_DSP, "Couldn't load function av_parser_close");
        return false;
    }
    return true;
}
