...
void Source::MixInto(QuadFrame32& dest, std::size_t intermediate_mix_id) const {
    ...
    for (std::size_t samplei{}; samplei < samples_per_frame; samplei++) {
...
void Source::ParseConfig(SourceConfiguration::Configuration& config,
    ...
    if (config.reset_flag) {
        ...
        Reset();
        LOG_TRACE(Audio_DSP, "(source: {}) reset", source_id);
    }
    if (config.partial_reset_flag) {
        ...
        LOG_TRACE(Audio_DSP, "(source: {}) partial_reset", source_id);
    }
    if (config.enable_dirty) {
        config.enable_dirty.Assign(0);
        state.enabled = config.enable != 0;
        LOG_TRACE(Audio_DSP, "(source: {}) enable={}", source_id, state.enabled);
    }
    if (config.sync_dirty) {
        config.sync_dirty.Assign(0);
        state.sync = config.sync;
        LOG_TRACE(Audio_DSP, "(source: {}) sync={}", source_id, state.sync);
    }
    if (config.rate_multiplier_dirty) {
        config.rate_multiplier_dirty.Assign(0);
        state.rate_multiplier = config.rate_multiplier;
        LOG_TRACE(Audio_DSP, "(source: {}) rate={}", source_id, state.rate_multiplier);
        if (state.rate_multiplier <= 0) {
            LOG_ERROR(Audio_DSP, "Was given an invalid rate multiplier: (source: {}) rate={}",
    ...
    if (config.adpcm_coefficients_dirty) {
        ...
        LOG_TRACE(Audio_DSP, "(source: {}) adpcm update", source_id);
    }
    if (config.gain_0_dirty) {
        ...
        LOG_TRACE(Audio_DSP, "(source: {}) gain 0 update", source_id);
    }
    if (config.gain_1_dirty) {
        ...
        std::transform(config.gain[1], config.gain[1] + state.gain[1].size(), state.gain[1].begin(),
                       [](const auto& coeff) { return static_cast<float>(coeff); });
        LOG_TRACE(Audio_DSP, "(source: {}) gain 1 update", source_id);
    }
    if (config.gain_2_dirty) {
        ...
        std::transform(config.gain[2], config.gain[2] + state.gain[2].size(), state.gain[2].begin(),
                       [](const auto& coeff) { return static_cast<float>(coeff); });
        LOG_TRACE(Audio_DSP, "(source: {}) gain 2 update", source_id);
    }
    if (config.filters_enabled_dirty) {
        ...
        LOG_TRACE(Audio_DSP, "(source: {}) enable_simple={} enable_biquad={}", source_id,
    ...
    if (config.format_dirty || config.embedded_buffer_dirty) {
        ...
        LOG_TRACE(Audio_DSP, "(source: {}) format={}", source_id,
    ...
    if (config.mono_or_stereo_dirty || config.embedded_buffer_dirty) {
        ...
        LOG_TRACE(Audio_DSP, "(source: {}) mono_or_stereo={}", source_id,
    ...
    if (config.dirty_raw)
        LOG_DEBUG(Audio_DSP, "(source: {}) remaining_dirty={:x}", source_id, config.dirty_raw);
    config.dirty_raw = 0;
}
...
bool Source::DequeueBuffer() {
    ...
    if (memory) {
        ...
        LOG_WARNING(Audio_DSP,
                    "(source: {}) buffer_id={} length={}: Invalid physical address {:#010x}",
    ...
    LOG_TRACE(Audio_DSP, "(source: {}) buffer_id={} from_queue={} current_buffer.size()={}",
