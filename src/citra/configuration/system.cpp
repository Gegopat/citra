...
#include <QMessageBox>
#include "citra/configuration/system.h"
...
ConfigurationSystem::ConfigurationSystem(QWidget* parent)
    : QWidget{parent}, ui{std::make_unique<Ui::ConfigurationSystem>()} {
    ui->setupUi(this);
    connect(ui->combo_birthmonth, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ConfigurationSystem::UpdateBirthdayComboBox);
    connect(ui->combo_init_clock, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ConfigurationSystem::UpdateInitTime);
    connect(ui->button_regenerate_console_id, &QPushButton::clicked, this,
            &ConfigurationSystem::RefreshConsoleID);
...
void ConfigurationSystem::LoadConfiguration(Core::System& system) {
    ...
    if (!enabled) {
        ...
        // This tab is enabled only when program isn't running (i.e. all services aren't
        // initialized).
...
void ConfigurationSystem::ReadSystemSettings() {
    // Set username
...
void ConfigurationSystem::ApplyConfiguration() {
    if (!enabled)
        return;
    bool modified{};
    // Apply username
...
void ConfigurationSystem::UpdateBirthdayComboBox(int birthmonth_index) {
...
void ConfigurationSystem::UpdateInitTime(int init_clock) {
...
void ConfigurationSystem::RefreshConsoleID() {
