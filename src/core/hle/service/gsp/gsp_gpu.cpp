...
static PAddr VirtualToPhysicalAddress(VAddr addr) {
    ...
    if (addr >= Memory::VRAM_VADDR && addr <= Memory::VRAM_N3DS_VADDR_END)
...
ResultCode GSP_GPU::SetBufferSwapImpl(u32 screen_id, const FrameBufferInfo& info) {
    ...
    auto phys_address_left{VirtualToPhysicalAddress(info.address_left)};
    auto phys_address_right{VirtualToPhysicalAddress(info.address_right)};
    if (info.active_fb == 0) {
...
void GSP_GPU::TriggerCmdReqQueue(Kernel::HLERequestContext& ctx) {
    ...
    for (unsigned thread_id{}; thread_id < 0x4; ++thread_id) {
        auto command_buffer{(CommandBuffer*)GetCommandBuffer(shared_memory, thread_id)};
