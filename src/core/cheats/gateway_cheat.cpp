...
struct State {
    ...
    u32 if_flag{};
    u32 loop_count{};
    std::size_t loop_back_line{};
    std::size_t current_line_nr{};
    bool loop_flag{};
};
...
GatewayCheat::CheatLine::CheatLine(const std::string& line) {
    constexpr std::size_t CheatValidLength{17};
    if (line.length() != CheatValidLength) {
        type = CheatType::Null;
        cheat_line = line;
    ...
    try {
        auto type_temp{line.substr(0, 1)};
...
void GatewayCheat::Execute(Core::System& system) {
    ...
    for (state.current_line_nr = 0; state.current_line_nr < cheat_lines.size();
        ...
        case CheatType::LoadOffset:
            ...
            LoadOffsetOp(line, state, memory);
            break;
        case CheatType::Loop:
