...
constexpr int kMaxSections{8};   ///< Maximum number of sections (files) in an ExeFs
constexpr int kBlockSize{0x200}; ///< Size of ExeFS blocks (in bytes)
...
Loader::ResultStatus NCCHContainer::LoadSectionExeFS(const char* name, std::vector<u8>& buffer) {
    ...
    for (unsigned section_number{}; section_number < kMaxSections; section_number++) {
        ...
        if (std::strcmp(section.name, name) == 0) {
            ...
            std::array<u8, 16> key;
            if (std::strcmp(section.name, "icon") == 0 || std::strcmp(section.name, "banner") == 0)
                key = primary_key;
            else
                key = secondary_key;
