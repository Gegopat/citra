...
Rasterizer::Rasterizer(Core::System& system)
    ...
    for (std::size_t i{}; i < texture_samplers.size(); ++i) {
...
void Rasterizer::SyncEntireState() {
    ...
    SyncGlobalAmbient();
    for (int light_index{}; light_index < 8; light_index++) {
...
static constexpr std::array<GLenum, 4> vs_attrib_types{
    ...
    GL_UNSIGNED_BYTE, // VertexAttributeFormat::UnsignedByte
...
bool Rasterizer::SetupVertexShader() {
...
bool Rasterizer::SetupGeometryShader() {
    const auto& regs{Pica::g_state.regs};
...
bool Rasterizer::AccelerateDrawBatchInternal(bool is_indexed, bool use_gs) {
    ...
    if (is_indexed) {
        ...
        if (index_buffer_size > INDEX_BUFFER_SIZE) {
            LOG_WARNING(Render, "Too large index input size {}", index_buffer_size);
            return false;
        }
        const u8* index_data{
...
void Rasterizer::NotifyPicaRegisterChanged(u32 id) {
    ...
    case PICA_REG_INDEX(rasterizer.cull_mode):
        ...
        break;
    // Clipping plane
    case PICA_REG_INDEX(rasterizer.clip_enabled):
        SyncClipEnabled();
        break;
    ...
    case PICA_REG_INDEX(rasterizer.viewport_depth_near_plane):
        ...
        break;
    // Depth buffering
    case PICA_REG_INDEX(rasterizer.depthmap_enabled):
        shader_dirty = true;
        break;
    // Blending
    case PICA_REG_INDEX(framebuffer.output_merger.alphablend_enabled):
        SyncBlendEnabled();
        break;
    ...
    case PICA_REG_INDEX(framebuffer.output_merger.depth_test_enabled):
        SyncDepthTest();
        SyncDepthWriteMask();
        SyncColorWriteMask();
        break;
    ...
    case PICA_REG_INDEX(lighting.light_enabled):
        break;
    ...
    case PICA_REG_INDEX_WORKAROUND(lighting.lut_data[7], 0x1cf):
        uniform_block_data.lighting_lut_dirty[regs.lighting.lut_config.type] = true;
        uniform_block_data.lighting_lut_dirty_any = true;
        break;
    }
}
...
bool Rasterizer::AccelerateTextureCopy(const GPU::Regs::DisplayTransferConfig& config) {
    ...
    dst_params.res_scale = src_surface->res_scale;
    dst_params.UpdateParams();
    // Since we're going to invalidate the gap if there is one, we'll have to load it first
...
void Rasterizer::SamplerInfo::Create() {
    sampler.Create();
...
void Rasterizer::SamplerInfo::SyncWithConfig(const Pica::TexturingRegs::TextureConfig& config) {
...
void Rasterizer::SyncClipEnabled() {
    state.clip_distance[1] = Pica::g_state.regs.rasterizer.clip_enabled != 0;
}
...
void Rasterizer::SyncDepthOffset() {
    float depth_offset{
        Pica::float24::FromRaw(Pica::g_state.regs.rasterizer.viewport_depth_near_plane)
            .ToFloat32()};
...
void Rasterizer::SyncBlendEnabled() {
    state.blend.enabled = (Pica::g_state.regs.framebuffer.output_merger.alphablend_enabled == 1);
}
...
void Rasterizer::SyncColorWriteMask() {
    ...
    state.color_mask.red_enabled = IsColorWriteEnabled(regs.framebuffer.output_merger.red_enabled);
    state.color_mask.green_enabled =
        IsColorWriteEnabled(regs.framebuffer.output_merger.green_enabled);
    state.color_mask.blue_enabled =
        IsColorWriteEnabled(regs.framebuffer.output_merger.blue_enabled);
    state.color_mask.alpha_enabled =
        IsColorWriteEnabled(regs.framebuffer.output_merger.alpha_enabled);
}
...
void Rasterizer::SyncDepthWriteMask() {
    ...
    state.depth.write_mask = (regs.framebuffer.framebuffer.allow_depth_stencil_write != 0 &&
                              regs.framebuffer.output_merger.depth_write_enabled)
                                 ? GL_TRUE
                                 : GL_FALSE;
}
...
void Rasterizer::SyncStencilTest() {
    ...
    state.stencil.test_enabled =
        regs.framebuffer.output_merger.stencil_test.enabled &&
...
void Rasterizer::SyncDepthTest() {
    const auto& regs{Pica::g_state.regs};
    state.depth.test_enabled = regs.framebuffer.output_merger.depth_test_enabled == 1 ||
                               regs.framebuffer.output_merger.depth_write_enabled == 1;
    state.depth.test_func =
        regs.framebuffer.output_merger.depth_test_enabled == 1
...
void Rasterizer::SyncTevConstColor(int stage_index,
...
void Rasterizer::SyncLightSpotDirection(int light_index) {
    ...
    GLvec3 spot_direction{{light.spot_x / 2047.0f, light.spot_y / 2047.0f, light.spot_z / 2047.0f}};
...
void Rasterizer::UploadUniforms(bool accelerate_draw, bool use_gs) {
