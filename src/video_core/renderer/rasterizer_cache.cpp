...
static const FormatTuple& GetFormatTuple(PixelFormat pixel_format) {
    const auto type{SurfaceParams::GetFormatType(pixel_format)};
...
static bool BlitTextures(GLuint src_tex, const MathUtil::Rectangle<u32>& src_rect, GLuint dst_tex,
    ...
    // TODO: use GL_NEAREST for shadow map texture
...
static bool FillSurface(const Surface& surface, const u8* fill_data,
    ...
    if (surface->type == SurfaceType::Color || surface->type == SurfaceType::Texture) {
        ...
        GLint value_int{static_cast<GLint>(value_32bit >> 24)};
        state.depth.write_mask = GL_TRUE;
...
SurfaceParams SurfaceParams::FromInterval(SurfaceInterval interval) const {
    ...
    const u32 tiled_size{static_cast<u32>(is_tiled ? 8 : 1)};
    const u32 stride_tiled_bytes{BytesInPixels(stride * tiled_size)};
    PAddr aligned_start{addr +
                        Common::AlignDown(boost::icl::first(interval) - addr, stride_tiled_bytes)};
    PAddr aligned_end{addr +
                      Common::AlignUp(boost::icl::last_next(interval) - addr, stride_tiled_bytes)};
...
MathUtil::Rectangle<u32> SurfaceParams::GetSubRect(const SurfaceParams& sub_surface) const {
    ...
    if (is_tiled) {
        const int x0{static_cast<int>(begin_pixel_index % (stride * 8)) / 8};
        const int y0{static_cast<int>(begin_pixel_index / (stride * 8)) * 8};
        // Top to bottom
        return MathUtil::Rectangle<u32>(x0, height - y0, x0 + sub_surface.width,
                                        height - (y0 + sub_surface.height));
    }
    const int x0{static_cast<int>(begin_pixel_index % stride)};
    const int y0{static_cast<int>(begin_pixel_index / stride)};
    // Bottom to top
...
SurfaceInterval SurfaceParams::GetCopyableInterval(const Surface& src_surface) const {
    ...
    const auto valid_regions{SurfaceRegions(GetInterval() & src_surface->GetInterval()) -
                             src_surface->invalid_regions};
...
void RasterizerCache::CopySurface(const Surface& src_surface, const Surface& dst_surface,
    ...
    if (src_surface->type == SurfaceType::Fill) {
        ...
        const u32 fill_offset{(boost::icl::first(copy_interval) - src_surface->addr) %
                              src_surface->fill_size};
        ...
        if (type == SurfaceType::Texture) {
            ...
            for (unsigned y{rect.bottom}; y < rect.top; ++y) {
                for (unsigned x{rect.left}; x < rect.right; ++x) {
                    auto vec4{Pica::Texture::LookupTexture(texture_src_data, x, height - 1 - y,
                                                           tex_info)};
...
void CachedSurface::DownloadGLTexture(const MathUtil::Rectangle<u32>& rect, GLuint read_fb_handle,
    ...
    std::size_t buffer_offset{(rect.bottom * stride + rect.left) *
                              GetGLBytesPerPixel(pixel_format)};
