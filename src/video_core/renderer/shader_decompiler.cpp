...
private:
    ...
    static ExitMethod ParallelExit(ExitMethod a, ExitMethod b) {
        if (a == ExitMethod::Undetermined)
            return b;
        else if (b == ExitMethod::Undetermined)
            return a;
        else if (a == b)
            return a;
    ...
    static ExitMethod SeriesExit(ExitMethod a, ExitMethod b) {
        ...
        else if (a == ExitMethod::AlwaysReturn)
            return b;
        else if (b == ExitMethod::Undetermined || b == ExitMethod::AlwaysEnd)
    ...
    ExitMethod Scan(u32 begin, u32 end, std::set<u32>& labels) {
        ...
        for (u32 offset{begin}; offset != end && offset != PROGRAM_END; ++offset) {
            ...
            case OpCode::Id::JMPU: {
                ...
                auto no_jmp{Scan(offset + 1, end, labels)};
                auto jmp{Scan(instr.flow_control.dest_offset, end, labels)};
            ...
            case OpCode::Id::CALL: {
                ...
                if (call.exit_method == ExitMethod::AlwaysEnd)
                    return exit_method = ExitMethod::AlwaysEnd;
                auto after_call{Scan(offset + 1, end, labels)};
            ...
            case OpCode::Id::LOOP: {
                ...
                if (loop.exit_method == ExitMethod::AlwaysEnd)
                    return exit_method = ExitMethod::AlwaysEnd;
                auto after_loop{Scan(instr.flow_control.dest_offset + 1, end, labels)};
            ...
            case OpCode::Id::IFC: {
                ...
                auto both{ParallelExit(if_sub.exit_method, else_method)};
...
private:
    ...
    void SetDest(const SwizzlePattern& swizzle, const std::string& reg, const std::string& value,
        ...
        auto dest{reg + (dest_num_components != 1 ? dest_mask_swizzle : "")};
        auto src{value};
    ...
    u32 CompileInstr(u32 offset) {
        ...
        case OpCode::Type::Arithmetic: {
            ...
            auto dest_reg{GetDestRegister(instr.common.dest.Value())};
            ...
            case OpCode::Id::DPHI: {
                ...
                if (opcode == OpCode::Id::DP3)
                    dot = sanitize_mul
                              ? "dot(vec3(sanitize_mul(" + src1 + ", " + src2 + ")), vec3(1.0))"
                              : "dot(vec3(" + src1 + "), vec3(" + src2 + "))";
                else {
                    auto src1_{(opcode == OpCode::Id::DPH || opcode == OpCode::Id::DPHI)
                                   ? "vec4(" + src1 + ".xyz, 1.0)"
                                   : src1};
                    dot = sanitize_mul ? "dot(sanitize_mul(" + src1_ + ", " + src2 + "), vec4(1.0))"
                                       : "dot(" + src1 + ", " + src2 + ")";
                }
            ...
            case OpCode::Id::CMP: {
                ...
                const auto op_x{instr.common.compare_op.x.Value()};
                const auto op_y{instr.common.compare_op.y.Value()};
        ...
        case OpCode::Type::MultiplyAdd: {
            if ((instr.opcode.Value().EffectiveOpCode() == OpCode::Id::MAD) ||
                ...
                SetDest(swizzle, dest_reg,
                        sanitize_mul ? "sanitize_mul(" + src1 + ", " + src2 + ") + " + src3
                                     : src1 + " * " + src2 + " + " + src3,
                        4, 4);
            } else {
        ...
        default: {
            ...
            case OpCode::Id::IFC:
            case OpCode::Id::IFU: {
                auto condition{instr.opcode.Value() == OpCode::Id::IFC
            ...
            case OpCode::Id::LOOP: {
                auto int_uniform{"uniforms.i[" + std::to_string(instr.flow_control.int_uniform_id) +
                                 "]"};
                shader.AddLine("address_registers.z = int(" + int_uniform + ".y);");
                auto loop_var{"loop" + std::to_string(offset)};
