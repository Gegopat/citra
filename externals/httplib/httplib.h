...
bool read_content(Stream &strm, T &x, Progress progress = Progress()) {
  if (has_header(x.headers, "Content-Length")) {
    ...
    if (len == 0) {
      const auto &encoding =
          get_header_value(x.headers, "Transfer-Encoding", "");
...
inline void SSLClient::add_client_cert_ASN1(std::vector<unsigned char> cert,
                                            std::vector<unsigned char> key) {
  if (is_valid()) {
    SSL_CTX_use_certificate_ASN1(ctx_, cert.size(), cert.data());
    SSL_CTX_use_PrivateKey_ASN1(SSL_FILETYPE_ASN1, ctx_, key.data(),
                                key.size());
  }
}
